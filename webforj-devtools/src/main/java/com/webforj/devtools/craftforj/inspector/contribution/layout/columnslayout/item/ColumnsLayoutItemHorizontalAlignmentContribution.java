...
/**
 * Contribution for the horizontal self-alignment of a columns layout item, applied via
 * {@code ColumnsLayout.setHorizontalAlignment(item, alignment)}.
 *
 * @author Hyyan Abo Fakher
 * @since 26.02
 */
@AutoService(FeatureHandler.class)
public class ColumnsLayoutItemHorizontalAlignmentContribution
    extends ColumnsLayoutItemContribution {

  private static final String STYLE_KEY = "justify-self";
  ...
  /** Creates the columns layout item horizontal alignment contribution. */
  @SuppressWarnings({"unchecked", "rawtypes"})
  public ColumnsLayoutItemHorizontalAlignmentContribution() {
    super(HasStyle.class, "SelfHorizontalAlignment", "setHorizontalAlignment");

    setBuilderConfig(b -> b.select(OPTIONS));
    setGetter(c -> readAlignment(c, STYLE_KEY));
    setSetter((parent, item, v) -> parent.setHorizontalAlignment((Component & HasStyle) item,
        parseAlignment(v)));

...
/**
 * A proxy for [HasComponents] to be used to add [Component] instances to the slots of other
 * `Components`.
 *
...
class MultiSlotSetter(block: @WebforjDsl HasComponents.() -> Unit) : HasComponents {
  ...
  /**
   * Adds the `Components` to the given [component] by invoking the [setter] with the backing list

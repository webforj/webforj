...
public class Terminal extends Div {
    ...
    protected void create(AbstractPanel p) {
        ...
        script = "function whenTerminalLoaded (callback) { if (typeof Terminal === 'undefined' ) {setTimeout (function () {whenTerminalLoaded (callback);}, 100);} else { callback (); }}";
        ...
        script=
           "whenTerminalLoaded(function() {window.term = new window.Terminal();"+
           "window.term.open(document.getElementById('"+uuid+"'));"+
           "window.term.write('\\x1B[1;3;31mDWCJ\\x1B[0m Terminal');"+
           "window.term.focus();"+
           "window.term.onKey("+
    ...
    public Terminal write(String chunk){
        ...
        String script=
                "whenTerminalLoaded(function() {window.term.write('"+chunk+"');})";
        hv.executeScript(script);
        return this;
    }
}
